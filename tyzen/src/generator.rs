...
impl Generator {
    ...
    pub fn run(self) -> std::io::Result<()> {
        ...
        {
            generate_full(self.output_path.to_str().unwrap(), self.config, write_command_declarations, |_| {})
        }
    }
}
...
pub fn generate_with_config(output_path: &str, config: GeneratorConfig) -> std::io::Result<()> {
    ...
    generate_full(output_path, config, write_tauri_commands, write_tauri_events)
}
